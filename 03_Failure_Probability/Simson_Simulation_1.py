...
input_data = open('Versagenswahrscheinlichkeit/Eingabe_Simson_1.txt','r')
...
def get_load_probability(x):
    load_probability = 1/(load_deviation*math.sqrt(2*math.pi))*math.exp(-(x-load_expectant)**2/(2*load_deviation**2))
...
with open('Versagenswahrscheinlichkeit/Ausgabe_Simson_1.txt','w') as output_data:
...
ax.grid()
plt.savefig("Versagenswahrscheinlichkeit/WVF_Belastung_Simson_1.png", dpi=300)

fig_2, ax = plt.subplots(figsize=(6,6))
ax.plot(y_history, material_probability)
ax.set(xlabel='y', ylabel='phi(y)')
ax.grid()
plt.savefig("Versagenswahrscheinlichkeit/WVF_Material_Simson_1.png", dpi=300)